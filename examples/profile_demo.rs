//! Example demonstrating the GhostProfile builder system.
//! 
//! Shows how to create customized browser fingerprint profiles
//! with the ergonomic builder pattern.

use ghostoxide::{Browser, BrowserConfig, GhostPage, GhostProfile, Gpu};
...
async fn main() -> Result<()> {
    ...
    let windows_profile = GhostProfile::windows().build();
    ...
    let custom_profile = GhostProfile::windows()
        .chrome_version(130)
    ...
    // macOS profile
    let mac_profile = GhostProfile::macos_arm()
        .gpu(Gpu::AppleM4Max)
        .memory_gb(64)
        .build();
    ...
    println!("\nTesting click_human()...");
    ghost.click_human(400.0, 300.0).await?;
    
    // Demonstrate type_text_with_typos
    println!("Testing type_text_with_typos()...");
    // ghost.type_text_with_typos("Hello world").await?;
